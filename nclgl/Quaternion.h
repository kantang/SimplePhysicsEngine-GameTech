...
public:
	...
	Vector3 GetNormal(){
		const Matrix4& orientation = ToMatrix();
		Vector3 normal = Vector3(orientation(0, 2), orientation(1, 2), orientation(2, 2));
		normal.Normalise();
		return normal;
	}

	void Plus(const Quaternion &a){ x += a.x; y += a.y; z += a.z; w += a.w; };
